...
async function fetchJettonDetails(ca) {
  try {
    // 1. Jetton metadata from TonAPI
    const metaRes = await fetch(`https://tonapi.io/v2/jetton-masters/${ca}`);
    const meta = await metaRes.json(); 

    // 2. Market data from Dexscreener
    const marketRes = await fetch(`https://api.dexscreener.com/latest/dex/tokens/ton/${ca}`);
    const marketJson = await marketRes.json();
    const market = marketJson.pairs?.[0] || {};

    // CUPKO Template
    const output = `
Avatar+
⚫️ CUPKO TON SCANNER ⚫️
============================

⚫️ ${meta.jetton?.name || "Unknown"} (${meta.jetton?.symbol || "?"})
💰 Price: $${market.priceUsd || "N/A"}
📈 24h Change: ➡️ ${market.priceChange?.h24 || "N/A"}

👤 OWNER & ADMIN INFO
• Owner: ${meta.admin?.address?.slice(0, 6)}...${meta.admin?.address?.slice(-4)}
• Full Address: ${meta.admin?.address || "N/A"}
• Verified: ${meta.jetton?.verification ? "✅ Verified" : "❌ Unverified"}
• Mintable: ${meta.jetton?.mintable ? "🔴 Yes" : "🟢 No"}

📊 MARKET DATA
• Market Cap: $${market.marketCap || "N/A"}
• FDV: $${market.fdv || "N/A"}
• Volume 24h: $${market.volume?.h24 || "N/A"}

⚫️ TOKEN DETAILS
• Total Supply: ${meta.jetton?.total_supply || "N/A"}
• Holders: ${meta.holders_count || "N/A"}
• Decimals: ${meta.jetton?.decimals || "N/A"}

👥 HOLDER ANALYSIS
⚠️ Holder data unavailable
• This could be due to API limitations or new token
• Manual verification recommended on TON explorers
• Check: tonwhales.com or tonscan.org

💰 TAX ANALYSIS
⚠️ TAX STATUS UNKNOWN - Verify manually

🛡️ SECURITY ANALYSIS
• Risk Level: 🔴 HIGH RISK
• Security Score: 20/100
⚠️ Top Risks:
• ⚠️ Token is mintable (supply can be increased)
• ⚠️ Token has admin control
• ...and 2 more

🌐 SOCIALS: ${market.info?.socials?.join(", ") || "🚨 None found"}
    `;
    console.log(output);
  } catch (err) {
...
// Example: Tether USDT on TON
fetchJettonDetails("EQCxE6mUtQJKFnGfaROTKOt1lZbDiiX1kCixRv7Nw2Id_sDs");
